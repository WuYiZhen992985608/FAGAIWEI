...
class LiaoningSpider(scrapy.Spider):
    ...
    def __init__(self, pagecount=2, *args, **kwargs):
        ...
        # 政策文件
        # self.home_indexurl = 'http://fgw.ln.gov.cn/zc/zcwjk/index.html'
        # self.page_indexurl = 'http://fgw.ln.gov.cn/zc/zcwjk/index_{}.html' #共4页
        # 政策解读
        self.home_indexurl = 'http://fgw.ln.gov.cn/zc/zcjd/index.html'
        self.page_indexurl = 'http://fgw.ln.gov.cn/zc/zcjd/index_{}.html'  #共3页
    ...
    def extract_item_page_link(self, response):
        links = []
        try:
            links = response.xpath('//*[@class="mod-body2 minhh600"]/ul/li/a/@href').getall()
    ...
    def get_title(self, response):
        title = response.css('.news-content-main h1 ::text').extract_first()
        return title

    def get_content(self, response):
        content = ''
        try:
            content = ''.join((response.xpath('//*[@class="TRS_Editor"]//text()')
                               or response.xpath('//*[@class="news-text"]//text()')
                               or response.xpath('//tbody//text()')).extract())
        except:
            pass
        return content
    ...
    def get_text(self, response):
        txt = ''
        try:
            txt = ''.join((response.xpath('//*[@class="TRS_Editor"]//text()')
                     or response.xpath('//*[@class="news-text"]//text()')
                     or response.xpath('//tbody//text()')).extract())
    ...
    def get_dates(self, response):
        dates = ''
        try:
            dates = re.search('发布时间：(\d+-\d+-\d+)',response.text).group(1)
        except:
            pass
        return dates
