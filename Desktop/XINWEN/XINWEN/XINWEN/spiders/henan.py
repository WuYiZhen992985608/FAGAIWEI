...
class HenanSpider(scrapy.Spider):
    ...
    def __init__(self, pagecount=1, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.page_count = self.get_input_pagecount(pagecount, 99999)
        self.domain = 'fgw.henan.gov.cn/'

        # # 规范性文件
        self.home_urls = ['http://fgw.henan.gov.cn/ztjc/gfxwj/',]  #共1页
        # 建设项目
        # self.home_urls = ['http://fgw.henan.gov.cn/ztjc/zdgz/index.html',
        #                     'http://fgw.henan.gov.cn/ztjc/zdgz/index_1.html',
        #                     'http://fgw.henan.gov.cn/ztjc/zdgz/index_2.html'] #共3页
        # 政策解读
        # self.home_urls = ['http://fgw.henan.gov.cn/ztjc/',]  #共1页
    ...
    def extract_item_page_link(self, response):
        links = []
        try:
            links = response.xpath('//*[contains(@class,"news")]/ul/li/a/@href').extract()
    ...
    def start_requests(self):
        try:
            for url in self.home_urls:
                yield scrapy.Request(url, callback=self.parse_homeindex, dont_filter=True)
        except Exception as e:
            logging.error(self.name + ": " + e.__str__())
            logging.exception(e)
    ...
    def get_title(self, response):
        title = response.xpath('//*[@class="yMain"]/p[1]/text()').extract_first()
        return title

    def get_content(self, response):
        content = ''
        try:
            content = ''.join(response.xpath('//*[@class="yMain"]/div[2]//text()').extract())
        except:
            pass
        return content
    ...
    def get_text(self, response):
        txt = ''
        try:
            txt = ''.join(response.xpath('//*[@class="yMain"]/div[2]//text()').extract())
    ...
    def get_dates(self, response):
        dates = ''
        try:
            dates = re.search('时间：(\d+-\d+-\d+)',response.text).group(1)
        except:
            pass
        return dates
