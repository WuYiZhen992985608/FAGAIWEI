...
class JilinSpider(scrapy.Spider):
    ...
    def __init__(self, pagecount=2, *args, **kwargs):
        ...
        self.domain = 'http://jldrc.jl.gov.cn/'

        # 政策法规
        # self.home_indexurl = 'http://jldrc.jl.gov.cn/zcfb/zcfg/index.html'
        # self.page_indexurl = 'http://jldrc.jl.gov.cn/zcfb/zcfg/index_{}.html' #共6页
        # 政策解读
        # self.home_indexurl = 'http://jldrc.jl.gov.cn/zcfb/zcjd/index.html'
        # self.page_indexurl = 'http://jldrc.jl.gov.cn/zcfb/zcjd/index_{}.html'  #共4页
        # 建设项目
        self.home_indexurl = 'http://jldrc.jl.gov.cn/zfgk/zdjsxm/jggs/index.html'
        self.page_indexurl = 'http://jldrc.jl.gov.cn/zfgk/zdjsxm/jggs/index_{}.html'  # 共6页
    ...
    def extract_item_page_link(self, response):
        links = []
        try:
            links = response.xpath('//*[@class="mian-1"]/ul/li/a/@href').getall()
    ...
    def get_title(self, response):
        title = response.css('.three-page-title ::text').extract_first()
        return title

    def get_content(self, response):
        content = ''
        try:
            content = ''.join((response.xpath('//*[@class="TRS_Editor"]/div//text()')
                               or response.xpath('//*[@class="TRS_Editor"]/p//text()')).extract())
        except:
            pass
        return content
    ...
    def get_text(self, response):
        txt = ''
        try:
            txt = ''.join((response.xpath('//*[@class="TRS_Editor"]/div//text()')
                               or response.xpath('//*[@class="TRS_Editor"]/p//text()')).extract())
    ...
    def get_symbol(self, response):
        symbol = ''
        try:
            symbol = response.xpath('//*[@class="TRS_Editor"]/p[1]/text()').extract_first()
        except:
            pass
        return symbol
