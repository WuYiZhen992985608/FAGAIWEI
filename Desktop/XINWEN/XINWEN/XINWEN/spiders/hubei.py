...
class HubeiSpider(scrapy.Spider):
    ...
    def __init__(self, pagecount=2, *args, **kwargs):
        ...
        self.domain = 'http://gw.hubei.gov.cn/'

        # # 政策解读
        self.home_indexurl = 'http://fgw.hubei.gov.cn/fbjd/zc/zcjd/index.shtml'
        self.page_indexurl = 'http://fgw.hubei.gov.cn/fbjd/zc/zcjd/index_{}.shtml' #共10页
        # 规划计划
        # self.home_indexurl = 'http://fgw.hubei.gov.cn/fbjd/xxgkml/ghjh/index.shtml'
        # self.page_indexurl = 'http://fgw.hubei.gov.cn/fbjd/xxgkml/ghjh/index_{}.shtml' #共3页


    def get_crawl_pagecount(self, response):
        try:
            totalpagecount = 0
            mh = re.compile('createPageHTML\((\d+).\s')
    ...
    def extract_item_page_link(self, response):
        links = []
        try:
            links = response.xpath('//*[@class="article-box"]/ul/li/a/@href').getall()
    ...
    def start_requests(self):
        header = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.80 Safari/537.36',
                  'Referer': 'http://fgw.hubei.gov.cn/material/styles/ui_xxgk2.css',}
        try:
            yield scrapy.Request(self.home_indexurl, callback=self.parse_homeindex,headers= header, dont_filter=True)
            yield scrapy.Request(self.home_indexurl, callback=self.parse_page_index,headers= header, dont_filter=True)
        except Exception as e:
            logging.error(self.name + ": " + e.__str__())
            logging.exception(e)
    ...
    def get_title(self, response):
        title = response.xpath('//*[@class="article"]/h2/text()').extract_first()
        return title

    def get_content(self, response):
        content = ''
        try:
            content = ''.join(response.xpath('//*[@class="article"]/div/div/div//text()').extract())
        except:
            pass
        return content
    ...
    def get_text(self, response):
        txt = ''
        try:
            txt = ''.join(response.xpath('//*[@class="article"]/div/div/div//text()').extract())
    ...
    def get_dates(self, response):
        dates = ''
        try:
            dates = response.xpath('tbody/tr[2]/td[2]/text()').extract_first()
        except:
            pass
        return dates
