...
class GjfgwSpider(scrapy.Spider):
    ...
    def __init__(self, pagecount=2, *args, **kwargs):
        ...
        self.domain = 'https://www.ndrc.gov.cn/'

        # # 发展改革委令
        # self.home_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/fzggwl/index.html'
        # self.page_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/fzggwl/index_{}.html' #共7页
        # # 规范性文件
        self.home_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/ghxwj/index.html'
        self.page_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/ghxwj/index_{}.html'  #共5页
        # 公告
        # self.home_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/gg/index.html'
        # self.page_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/gg/index_{}.html' #共20页
        # 规划文本
        # self.home_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/ghwb/index.html'
        # self.page_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/ghwb/index_{}.html' #共9页
        # # 通知
        # self.home_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/tz/index.html'
        # self.page_indexurl = 'https://www.ndrc.gov.cn/xxgk/zcfb/tz/index_{}.html'  #共20页
        # 其他
    ...
    def get_crawl_pagecount(self, response):
        try:
            totalpagecount = 0
            mh = re.compile('createPageHTML.(\d+),\s\d,\s')
    ...
    def extract_item_page_link(self, response):
        links = []
        try:
            links = response.css("ul.u-list a::attr(href)").getall()
            # links = [self.domain + l[2:] for l in links]
    ...
    def get_title(self, response):
        title = ((response.xpath('//*[@class="TRS_Editor"]/*[@align="center"][1]//text()')
                 or response.xpath('//*[@class="TRS_Editor"]/*[@align="center"][1]//text()'))
                 or (response.xpath('//*[@class="TRS_Editor"]/div[1]/strong//text()')
                 or response.xpath('//*[@class="article_l"]/h2/text()'))
                 or (response.xpath('//*[@class="article_l"]/div[1]/div[1]/strong//text()')
                 or response.xpath('//*[@id="activity-name"]//text()'))
                 or (response.xpath('//*[@class="article_l"]/div[1]/div[1]/span//text()')
                 or response.xpath('//*[@class="article_l"]/div[1]/p[1]/span//text()'))
                 or response.xpath('//*[@class="article_l"]/div[1]/div[1]/span/strong/text()')).extract_first()
        return title

    def get_content(self, response):
        content = ''
        try:
            content = ''.join((response.css('.TRS_Editor ::text') or response.xpath('//*[@class="article_l"]/div[1]//text()')).extract())
        except:
            pass
        return content
    ...
    def get_text(self, response):
        txt = ''
        try:
            txt = ''.join((response.css('.TRS_Editor ::text') or response.xpath('//*[@class="article_l"]/div[1]//text()')).extract())
    ...
    def get_symbol(self, response):
        symbol = ''
        try:
            symbol = response.xpath('//*[@class="center"//text()').extract_first()
        except:
            pass
        return symbol
    ...
    def get_dates(self, response):
        dates = ''
        try:
            dates = response.xpath('//div[@class="time"]/text()').extract_first().split('：')[1].strip()
        except:
            pass
        return dates
