...
class ShanghaiSpider(scrapy.Spider):
    ...
    def __init__(self, pagecount=2, *args, **kwargs):
        ...
        self.domain = 'http://fgw.ln.gov.cn/'

        # 本地法规
        # self.home_indexurl = 'http://fgw.sh.gov.cn/bswj/index.html'
        # self.page_indexurl = 'http://fgw.sh.gov.cn/bswj/index_{}.html' #共3页
        # 规范性文件
        # self.home_indexurl = 'http://fgw.sh.gov.cn/gfxwj/index.html'
        # self.page_indexurl = 'http://fgw.sh.gov.cn/gfxwj/index_{}.html'  #共6页
        # 规划计划
        # self.home_indexurl = 'http://fgw.sh.gov.cn/ggjg/index.html'
        # self.page_indexurl = 'http://fgw.sh.gov.cn/ggjg/index_{}.html'  # 共5页
        # 政策解读
        self.home_indexurl = 'http://fgw.sh.gov.cn/zcwjjd/index.html'
        self.page_indexurl = 'http://fgw.sh.gov.cn/zcwjjd/index_{}.html'  #共8页

    def get_crawl_pagecount(self, response):
        try:
            totalpagecount = 0
            mh = re.compile('totalPage: (\d+),')
    ...
    def extract_item_page_link(self, response):
        links = []
        try:
            links = response.xpath('//*[contains(@class,"xwzx_list") or contains(@class,"tagContentghjh")]/ul/li/a/@href').getall()
    ...
    def parse_homeindex(self, response):
        try:
            ...
            for page in range(2, pagecount+1):  # 第二页index_1.htm
    ...
    def get_title(self, response):
        title = response.css('.xwzx_title0 h3 ::text').extract_first()
        return title

    def get_content(self, response):
        content = ''
        try:
            content = ''.join(response.xpath('//*[@id="ivs_content"]//text()').extract())
        except:
            pass
        return content
    ...
    def get_text(self, response):
        txt = ''
        try:
            txt = ''.join(response.xpath('//*[@id="ivs_content"]//text()').extract())
    ...
    def get_symbol(self, response):
        symbol = ''
        try:
            symbol = response.xpath('//*[@class="xwzx_time1"/ul/li[1]/text()').extract_first()
        except:
            pass
        return symbol
