...
class HunanSpider(scrapy.Spider):
    ...
    def __init__(self, pagecount=2, *args, **kwargs):
        ...
        self.domain = 'http://fgw.hunan.gov.cn/'

        # 法律法规
        # self.home_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/zcfg/gjjfg/index.html'
        # self.page_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/zcfg/gjjfg/index_{}.html'  #共14页
        # 政策规章
        # self.home_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/zcfg/dfxfg/index.html'
        # self.page_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/zcfg/dfxfg/index_{}.html' #共6页
        # 规范性文件
        # self.home_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/zcfg/gfxwj/index.html'
        # self.page_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/zcfg/gfxwj/index_{}.html' #共6页
        # 规划计划
        # self.home_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/ghjh/index.html'
        # self.page_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/ghjh/index_{}.html'  # 共4页
        # 建设项目
        self.home_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/zdly/zdjsxm/index.html'
        self.page_indexurl = 'http://fgw.hunan.gov.cn/fgw/xxgk_70899/zdly/zdjsxm/index_{}.html'  # 共12页


    def get_crawl_pagecount(self, response):
        try:
            totalpagecount = 0
            mh = re.compile("\'paging\',(\d+),")
    ...
    def extract_item_page_link(self, response):
        links = []
        try:
            links = response.xpath('//*[@class="table"]/tbody/tr/td[2]/a/@href').getall()
            links = [self.domain + l[1:] for l in links]
    ...
    def get_title(self, response):
        title = response.css('.tys-main-zt h3 ::text').extract_first()
        return title

    def get_content(self, response):
        content = ''
        try:
            content = ''.join((response.xpath('//*[@class="tys-main-zt"]/div[3]//text()')
                    or response.xpath('//*[@class="tys-main-zt-show"]//text()')).extract())
        except:
            pass
        return content
    ...
    def get_text(self, response):
        txt = ''
        try:
            txt = ''.join((response.xpath('//*[@class="tys-main-zt"]/div[3]//text()')
                    or response.xpath('//*[@class="tys-main-zt-show"]//text()')).extract())
    ...
    def get_dates(self, response):
        dates = ''
        try:
            dates = re.search('发文日期：(\d+-\d+-\d+)\s',response.text).group(1)
        except:
            pass
        return dates
